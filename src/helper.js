...
const deploy = (version, env) => {
  ...
  return _getRemoteVersions().then(versions => {
    if (!isDaily && !!~versions.indexOf(version)) {
      return Promise.reject(new Error(`The version [${version}] is already existing, try another`))
    }
...
const getCurrentVersion = () => {
  return new Promise((resolve, reject) => {
    exec('git rev-parse --abbrev-ref HEAD', { cwd: process.cwd() }, (err, stdout) => {
      ...
      const match = /^daily\/(\d+\.\d+\.\d+)$/.exec(out)
      if (match && match[1]) return resolve(match[1])
      return reject(new Error(`The format of branch (${out}) is wrong, should like this: daily/x.y.z`))
    })
  })
}
...
const getLatestVersion = () => {
  return _getRemoteVersions().then(versions => {
    if (versions.length === 0) {
      return Promise.reject(new Error('No version in remote, please use Custom Version'))
    }
...
const checkRemoteMaster = (remote = 'origin') => {
  ...
  return new Promise((resolve, reject) => {
    ...
    exec(cmd, { cwd: process.cwd() }, (err, stdout) => {
      ...
      return reject(new Error('The current branch is behind the master, please merge at first'))
    })
  })
}
